...
int main() {
    ...
    if (fork() == 0) {
        ...
        sem_wait(sem_id);
        strcat(shm_buf, "Message from server 1\n");
        sem_signal(sem_id);
    } else {
        // Родительский процесс сервера 2
        if (fork() == 0) {
            // Дочерний процесс пишет в общий сегмент
            sem_wait(sem_id);
            strcat(shm_buf, "Message from server 2\n");
            sem_signal(sem_id);
        } else {
            // Родительский процесс клиента
            // Ожидаем завершения обоих серверных процессов
            wait(NULL);
            wait(NULL);

            // Клиент читает из общего сегмента
